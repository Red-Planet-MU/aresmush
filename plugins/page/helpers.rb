module AresMUSH
  module Page
    ...
    def self.send_page(enactor, recipients, message, client)
      message = PoseFormatter.format(enactor.name_and_alias, message.strip)
    ...
    def self.build_page_web_data(thread, enactor, lazy_load = false)
      if (lazy_load)
        ...
        messages = thread.sorted_messages.map { |p| {
            ...
            brief_message: Website.format_markdown_for_html(p.message)[0, ("<p>"+p.author.name+" says,").size] == "<p>"+p.author.name+" says," ? Website.format_markdown_for_html(p.message.sub(p.author.name+" says, ","")) : Website.format_markdown_for_html(p.message),
            id: p.id,
      ...
      is_unread = Page.is_thread_unread?(thread, enactor)
      {
         ...
         short_title: thread.title_customized(enactor).sub(enactor.name+" ","").gsub(" ", ", "),
         enabled: true,
         can_join: true,
         can_talk: true,
         muted: false,
