  ...
  module FS3Combat
      ...
      Global.dispatcher.spawn("Combat Turn", nil) do
        begin
          ...
          data = {
           ...
           Global.client_monitor.notify_web_clients(:new_combat_turn, "#{data.to_json}", true) do |c|
    ...
    def self.build_combatant_summary_data(combat, combatant, viewer)
...

      {
        ...
        action: combatant.action ? Website.format_markdown_for_html(combatant.action.print_action_short) : "",
    ...
    def self.damage_list_web_data(model, include_healed = true)
      model.damage
        ...
        .map { |d| {
                  ...
                  healing_progress: (FS3Combat.healing_points(d.current_severity) - d.healing_points) > 0 ? (((FS3Combat.healing_points(d.current_severity) - d.healing_points).to_f / FS3Combat.healing_points(d.current_severity).to_f )*5).round : nil
                  }}
    end
