    ...
    describe TDSSkills do
      ...
      describe :emit_results do
        ...
        context "private roll" do
          it "should emit to the client" do
            ...
            TDSSkills.emit_results("test", @main_client, @room, true)
          end
        ...
        context "public roll" do
          ...
          it "should emit to the room" do
            ...
            TDSSkills.emit_results("test", @main_client, @room, false)
          end
      ...
      describe :roll_ability do
        ...
        it "should roll ability" do
          ...
          expect(TDSSkills).to receive(:dice_to_roll_for_ability).with(@char, roll_params) { 5 }
          expect(TDSSkills.roll_ability(@char, roll_params)).to eq [6, 5, 5, 1, 5]
        end
      end
    
      describe :roll_dice do
        it "should roll the specified number of dice" do
          expect(TDSSkills.roll_dice(4)).to eq [ 6, 5, 5, 1 ]
        end
        
        it "should always roll 1 die even if asked for 0 or less" do
          expect(TDSSkills.roll_dice(0)).to eq [6]
        end
        
        it "should not allow giant die rolls" do
          expect(TDSSkills.roll_dice(99)).to eq [8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8]
        end
      end
    
      describe :get_success_level do
        it "should return number of successes if there are any" do
          expect(TDSSkills.get_success_level([7, 1, 3, 8])).to eq 2
        end
        
        it "should return a botch if there are more than two ones and no successes" do
          expect(TDSSkills.get_success_level([4, 1, 1, 1])).to eq -1
        end
        
        it "should return a failure if there are no successes but less than two ones." do
          expect(TDSSkills.get_success_level([4, 1, 3, 4])).to eq 0
        end
      end
