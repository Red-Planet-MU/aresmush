  ...
  module TDSSkills
    describe TDSSkills do
      ...
      before do
        ...
        allow(TDSSkills).to receive(:advantage_names) { [ "Rank", "Resources" ] }
      end
      ...
      describe :ability_rating do
        ...
        it "should get skills that exist" do
          expect(TDSSkills.ability_rating(@char, "English")).to eq 1
          expect(TDSSkills.ability_rating(@char, "Firearms")).to eq 2
          expect(TDSSkills.ability_rating(@char, "Basketweaving")).to eq 3
          expect(TDSSkills.ability_rating(@char, "Brawn")).to eq 4
          expect(TDSSkills.ability_rating(@char, "Rank")).to eq 3
        end
        ...
        it "should get skills that don't exit" do
          ...
          expect(TDSSkills.ability_rating(@char, "Mind")).to eq 0
          expect(TDSSkills.ability_rating(@char, "Art")).to eq 0
          expect(TDSSkills.ability_rating(@char, "Resources")).to eq 0
        end
      ...
      describe :get_linked_attr do
        ...
        it "should get linked for an action skill from the skill config" do
          expect(TDSSkills.get_linked_attr("Firearms")).to eq "Brawn"
        end

        it "should return nothing for attribute" do
          expect(TDSSkills.get_linked_attr("Mind")).to be_nil
        end
        
        it "should default for langs and background skills" do
          expect(TDSSkills.get_linked_attr("Basketweaving")).to eq "Wits"
          expect(TDSSkills.get_linked_attr("Spanish")).to eq "Wits"
        end
        
        it "should default for advantages" do
          expect(TDSSkills.get_linked_attr("Rank")).to eq "Wits"
        end
