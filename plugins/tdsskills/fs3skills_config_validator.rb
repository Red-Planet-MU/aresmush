module AresMUSH
  module TDSSkills
    class TDSSkillsConfigValidator
      attr_accessor :validator
      
      def initialize
        @validator = Manage::ConfigValidator.new("tdsskills")
      end
      ...
      def validate
        ...
        begin
          ...
          @validator.add_error "Unknown TDSSkills config error.  Fix other errors first and try again. #{ex} #{ex.backtrace[0, 3]}"
        end
      ...
      def check_attributes
        abilities = Global.read_config('tdsskills', 'attributes')
        abilities.each do |s|
          name = s['name'] || "Missing Name"
          ['name', 'desc'].each do |prop|
            verify_property_exists(name, s, prop)
          end
          if (TDSSkills.check_ability_name(name))
            @validator.add_error "tdsskills:attributes #{name} cannot contain special characters."
          end
        end
        if (!TDSSkills.attr_names.include?(Global.read_config('tdsskills', 'default_linked_attr')))
          @validator.add_error "tdsskills:default_linked_attr is not a valid attribute."
        end
      end
      
      def check_action_skills
        action_skills = Global.read_config('tdsskills', 'action_skills')
        action_skills.each do |s|
          ...
          if (TDSSkills.check_ability_name(name))
            @validator.add_error "tdsskills:action_skills #{name} cannot contain special characters."
          end
          (s['specialties'] || []).each do |spec|
            if (TDSSkills.check_ability_name(spec))
              @validator.add_error "tdsskills:action_skills #{spec} cannot contain special characters."
            end
          end
          if (!TDSSkills.attr_names.include?(s['linked_attr']))
            @validator.add_error "tdsskills:action_skills #{spec} cannot contain special characters."
          end
        end
      end
      ...
      def check_advantages
        abilities = Global.read_config('tdsskills', 'advantages')
        abilities.each do |s|
          name = s['name'] || "Missing Name"
          ['name', 'desc'].each do |prop|
            verify_property_exists(name, s, prop)
          ...
          if (TDSSkills.check_ability_name(name))
            @validator.add_error "tdsskills:advantages #{name} cannot contain special characters."
          end
        end
      end
      
      def check_background_skills
        abilities = Global.read_config('tdsskills', 'background_skills')
        abilities.each do |name, desc|
          if (TDSSkills.check_ability_name(name))
            @validator.add_error "tdsskills:background_skills #{name} cannot contain special characters."
          end
        end
      end
      
      def check_languages
        abilities = Global.read_config('tdsskills', 'languages')
        abilities.each do |s|
          name = s['name'] || "Missing Name"
          verify_property_exists(name, s, 'name')
          if (TDSSkills.check_ability_name(name))
            @validator.add_error "tdsskills:languages #{s['name']} cannot contain special characters."
          end
        end
      end
      
      def check_xp
        xp = Global.read_config('tdsskills', 'xp_costs')
        categories = ['language', 'action', 'background', 'attribute', 'advantage']
        categories.each do |cat|
          if (!xp.keys.include?(cat))
            @validator.add_error "tdsskills:xp_costs is missing category #{cat}."
          end
        end
        
        xp.each do |type, data|
          if (!categories.include?(type))
            @validator.add_error "tdsskills:xp_costs #{type} is not a valid ability category."
          end
          
          data.each do |level, cost|
            if (level.to_i < 0 || level.to_i > 7)
              @validator.add_error "tdsskills:xp_costs #{type} level #{level} is not a valid level."
            end
            if (level.to_i == 0 && cost != 1)
              next if type == "advantage" && !Global.read_config("tdsskills", "allow_advantages_xp")
              @validator.add_error "tdsskills:xp_costs #{type} level 0->1 always costs 1XP no matter what you put here."
            end
          end
      ...
      def check_misc
        luck_for_scene = Global.read_config('tdsskills', 'luck_for_scene')
        luck_for_scene.each do |scenes, luck|
          if (scenes.to_i < 0)
            @validator.add_error "tdsskills:luck_for_scene #{scenes} is not a valid number of scenes."
          end
          if (!(luck.kind_of?(Integer) || luck.kind_of?(Float)))
            @validator.add_error "tdsskills:luck_for_scene #{luck} is not a valid number of luck points."
          end
        end
        ...
        max_attrs.each do |level, num|
          if (level.to_i == 0)
            @validator.add_error "tdsskills:max_attrs_at_or_above #{level} is not a valid number."
          end

          if (level.to_i < 1 || level.to_i > 4)
            @validator.add_error "tdsskills:max_attrs_at_or_above #{level} must be from 1-4."
          end

          if (num.to_i < 0)
            @validator.add_error "tdsskills:max_attrs_at_or_above #{num} must be greater than 0."
          end
        end
        ...
        max_skills.each do |level, num|
          if (level.to_i == 0)
            @validator.add_error "tdsskills:max_skills_at_or_above #{level} is not a valid number."
          end

          if (level.to_i < 1 || level.to_i > 8)
            @validator.add_error "tdsskills:max_skills_at_or_above #{level} must be from 1-8."
          end

          if (num.to_i < 0)
            @validator.add_error "tdsskills:max_skills_at_or_above #{num} must be greater than 0."
          end
        end
        
        start_skills = Global.read_config('tdsskills', 'starting_skills')
        start_skills.each do |name, data|
          if (name == "Everyone")
            ...
            if (!group)
              @validator.add_error "tdsskills:starting_skills #{name} is not a valid group."
            end
      ...
      def validate_starting_skills_list(skills, name)
        if (!skills || !skills.kind_of?(Hash))
          @validator.add_error "tdsskills:starting_skills #{name} is missing a skills list."
        end
      
        skills.each do |skill, rating|
          if (TDSSkills.check_ability_name(skill))
            @validator.add_error "tdsskills:starting_skills #{skill} is an invalid skill name."
          end
          min_rating = Global.read_config("tdsskills", "allow_incapable_action_skills") ? 0 : 1
          if (!rating.kind_of?(Integer) || rating.to_i < min_rating || rating.to_i > 8)
            @validator.add_error "tdsskills:starting_skills #{skill} has an invalid rating."
          end
        end
      end
      ...
      def verify_property_exists(name, data, prop)
        if (!data[prop])
          @validator.add_error "tdsskills #{name} missing #{prop}." 
        end
      end
