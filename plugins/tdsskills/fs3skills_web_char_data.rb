  ...
  module TDSSkills
    class WebCharDataBuilder
      def build(char, viewer)
        ...
        show_sheet = TDSSkills.can_view_sheets?(viewer) || is_owner
      ...
      def get_ability_list(list, include_specs = false)        
        list.to_a.sort_by { |a| a.name }.map { |a| 
          { 
            ...
            linked_attr: include_specs ? TDSSkills.get_linked_attr(a.name)[0..2].upcase : nil
          }}
      end
      ...
      def get_xp_list(char, list)
        list.to_a.sort_by { |a| a.name }.map { |a| {
          ...
          days_to_learn: TDSSkills.days_to_next_learn(a)
        }}
      end
    end
  end
