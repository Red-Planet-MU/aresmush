  ...
  module TDSSkills
    class SetAbilityCmd
      ...
      def check_can_set
        ...
        return nil if TDSSkills.can_manage_abilities?(enactor)
      ...
      def handle
        ClassTargetFinder.with_a_character(self.name, client, enactor) do |model|        
          ...
          error = TDSSkills.check_rating(self.ability_name, new_rating)
          if (error)
            client.emit_failure error
            return
          end
          
          error = TDSSkills.set_ability(model, self.ability_name, new_rating)
          if (error)
            client.emit_failure error
          else
            client.emit_success TDSSkills.ability_raised_text(model, self.ability_name)
          end
