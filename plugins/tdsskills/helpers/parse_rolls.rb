module AresMUSH
  module TDSSkills
    ...
    def self.dice_to_roll_for_ability(char, roll_params)
      ...
      linked_attr = roll_params.linked_attr || TDSSkills.get_linked_attr(ability)
      ability_type = TDSSkills.get_ability_type(ability)
      skill_rating = TDSSkills.ability_rating(char, ability)
      ...
      apt_rating = linked_attr ? TDSSkills.ability_rating(char, linked_attr) : 0
    ...
    def self.parse_and_roll(char, roll_str)
      if (roll_str.is_integer?)
        ...
        die_result = TDSSkills.roll_dice(dice)
      else
        roll_params = TDSSkills.parse_roll_params roll_str
        if (!roll_params)
          return nil
        end
        die_result = TDSSkills.roll_ability(char, roll_params)
      end
      die_result
    end
    ...
    def self.parse_roll_params(str)
      ...
      if (linked_attr)
        ability_type = TDSSkills.get_ability_type(linked_attr)
