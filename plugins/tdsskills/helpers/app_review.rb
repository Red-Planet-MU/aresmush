module AresMUSH
  module TDSSkills
    ...
    def self.backgrounds_review(char)
      TDSSkills.min_item_review(char.fs3_background_skills.count, "min_backgrounds", "fs3skills.backgrounds_added")
    end
    ...
    def self.ability_rating_review(char)
      ...
      max_skills.each do |rating, limit|
        error = TDSSkills.check_high_abilities(char.fs3_action_skills, rating, limit, 'fs3skills.action_skills_above')
        too_high << error if error
      end
      
      max_attrs = Global.read_config('fs3skills', 'max_attrs_at_or_above')
      max_attrs.each do |rating, limit|
        error = TDSSkills.check_high_abilities(char.fs3_attributes, rating, limit, 'fs3skills.attributes_above')
        too_high << error if error
      end      
