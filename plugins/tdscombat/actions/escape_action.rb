module AresMUSH
  module TDSCombat
    class EscapeAction < CombatAction
      ...
      def prepare
        return t('tdscombat.not_subdued') if !self.combatant.is_subdued?
        return nil
      end

      def print_action
        t('tdscombat.escape_action_msg_long', :name => self.name, :target => self.subduer_name)
      end
      
      def print_action_short
        t('tdscombat.escape_action_msg_short', :target => self.subduer_name)
      end
      ...
      def resolve
        ...
        if (!self.combatant.is_subdued?)
          reset_subdue
          messages << t('tdscombat.escape_action_success', :name => self.name, :target => self.subduer_name)
        else  
          ...
          margin = TDSCombat.determine_attack_margin(self.subduer, self.combatant)
          if (margin[:hit])
            messages << t('tdscombat.escape_action_failed', :name => self.name, :target => self.subduer_name)
          else
            messages << t('tdscombat.escape_action_success', :name => self.name, :target => self.subduer_name)
            self.reset_subdue
          end
