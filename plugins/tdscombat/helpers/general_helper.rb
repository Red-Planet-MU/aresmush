module AresMUSH
  module TDSCombat
    ...
    def self.can_manage_combat_from_web?(actor, combat)
      AresCentral.play_screen_alts(actor).any? { |a| TDSCombat.can_manage_combat?(a, combat) }
    end
    ...
    def self.combatant_types
      Global.read_config("tdscombat", "combatant_types")
    end
    
    def self.stances
      Global.read_config("tdscombat", "stances")
    end
    
    def self.default_combatant_type
      Global.read_config("tdscombat", "default_type")
    end
    ...
    def self.mounts_allowed?
      Global.read_config("tdscombat", "allow_mounts")
    end
    ...
    def self.is_in_combat?(name)
      !!TDSCombat.combat(name)
    end
    
    def self.combat(name)
      TDSCombat.combats.select { |c| c.has_combatant?(name) }.first
    end
    ...
    def self.emit_to_combat(combat, message, npcmaster = nil, scene_pose = false, filter_for_screenreader = false)
      ...
      combat.combatants.each do |combatant|
        TDSCombat.emit_to_combatant(combatant, message, filter_for_screenreader)
      end
    end
    ...
    def self.emit_to_organizer(combat, message, npcmaster = nil)
      ...
      if (client)
        client.emit t('tdscombat.organizer_emit', :message => message)
      end
    end
    ...
    def self.emit_to_combatant(combatant, message, filter_for_screenreader = false)
      ...
      if (client)
        ...
        client_message.split("\n").each do |part|
          ...
          client.emit t('tdscombat.combat_emit', :message => part)
        end
      end
    end
    
    def self.combatant_type_stat(type, stat)
      type_config = TDSCombat.combatant_types[type]
      type_config[stat]
    end
    ...
    def self.npc_type_names
      Global.read_config("tdscombat", "npc_types").keys.map { |n| n.titlecase }
    end
    
    def self.default_npc_type
      Global.read_config('tdscombat', 'default_npc_type') || 'Goon'
    end
    ...
    def self.get_initiative_order(combat)
      ability = Global.read_config("tdscombat", "initiative_skill")
      order = []
      combat.active_combatants.each do |c|
        roll = TDSCombat.roll_initiative(c, ability)
    ...
    def self.with_a_combatant(name, client, enactor, &block)      
      ...
      if (!combatant)
        client.emit_failure t('tdscombat.not_in_combat', :name => name)
        return
      end
    ...
    def self.npcmaster_text(name, actor)
      return nil if !actor
      actor.name == name ? nil : t('tdscombat.npcmaster_text', :name => actor.name)
    end
    ...
    def self.new_turn(enactor, combat)
      ...
      if (combat.first_turn)
        combat.active_combatants.select { |c| c.is_npc? && !c.action }.each_with_index do |c, i|
          TDSCombat.ai_action(combat, c)
        end
        TDSCombat.emit_to_combat combat, t('tdscombat.new_turn', :name => enactor.name)
        combat.update(first_turn: false)
        return
      end
      
      TDSCombat.emit_to_combat combat, t('tdscombat.starting_turn_resolution', :name => enactor.name)
      combat.update(turn_in_progress: true)
      combat.update(everyone_posed: false)

      Global.dispatcher.spawn("Combat Turn", nil) do
        begin
          initiative_order = TDSCombat.get_initiative_order(combat)
          ...
          TDSCombat.emit_to_combat combat, all_messages.join("\n"), nil, true, true

          combat.log "---- Resolutions ----"          
          combat.active_combatants.each { |c| TDSCombat.reset_for_new_turn(c) }
          ...
          combat_data = TDSCombat.build_combat_web_data(combat, nil)
    ...
    def self.build_combat_web_data(combat, viewer)
      can_manage = viewer && AresCentral.play_screen_alts(viewer).any? { |a| TDSCombat.can_manage_combat?(a, combat) }
        ...
        .map { |team, members| 
          { 
            ...
            combatants: members.map { |c| 
              TDSCombat.build_combatant_summary_data(combat, c, viewer)
            }
          }
        }
    ...
    def self.build_combatant_summary_data(combat, combatant, viewer)
      can_manage = TDSCombat.can_manage_combat?(viewer, combat)

      {
        ...
        damage: TDSCombat.damage_list_web_data(combatant.associated_model, false),
    ...
    def self.damage_list_web_data(model, include_healed = true)
      model.damage
        ...
        .map { |d| {
                  ...
                  original_severity: MushFormatter.format(TDSCombat.display_severity(d.initial_severity)),
                  severity: MushFormatter.format(TDSCombat.display_severity(d.current_severity))
                  }}
    end
  end
end