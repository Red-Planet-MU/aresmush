module AresMUSH
  module TDSCombat
    ...
    def self.display_severity(value)
      case value
      when 'HEAL'
        t('tdscombat.healed_wound')
      when 'GRAZE'
        t('tdscombat.graze_wound')
      when 'FLESH'
        t('tdscombat.flesh_wound')
      when 'IMPAIR'
        t('tdscombat.impaired_wound')
      when 'INCAP'
        t('tdscombat.incap_wound')
      end
    end
    ...
    def self.can_inflict_damage(enactor, target)
      ...
      return true if TDSCombat.can_manage_damage?(enactor)
    ...
    def self.inflict_damage(target, severity, desc, is_stun = false, is_mock = false)
      ...
      params = {
        ...
        :healing_points => TDSCombat.healing_points(severity),
        :is_stun => is_stun, 
        :is_mock => is_mock
      }
      ...
      TDSCombat.award_damage_achievement(target, is_mock)
     end
     
     def self.healing_points(wound_level)
       Global.read_config("tdscombat", "healing_points", wound_level)
     end
     ...
     def self.treat_skill
       Global.read_config("tdscombat", "treat_skill")
     end
     
     def self.healing_skill
       Global.read_config("tdscombat", "healing_skill")
     end
     
     def self.max_patients(char)
       rating = FS3Skills.ability_rating(char, TDSCombat.healing_skill)
       rating / 2
     end
     ...
     def self.heal_wounds(char)
       ...
       ability = Global.read_config("tdscombat", "recovery_skill")
     ...
     def self.worst_treatable_wound(char_or_npc)
       ...
       treatable.sort_by { |d| TDSCombat.damage_severities.index(d.current_severity) }.reverse.first
     end
     
     def self.treat(patient_char_or_npc, healer_char_or_npc)
       wound = TDSCombat.worst_treatable_wound(patient_char_or_npc)
       healer_name = healer_char_or_npc.name
       patient_name = patient_char_or_npc.name
       
       if (!wound)
         return t('tdscombat.no_treatable_wounds', :healer => healer_name, :patient => patient_name)
       end
       
       skill = TDSCombat.treat_skill 
       ...
       if (successes <= 0)
         return t('tdscombat.treat_failed', :healer => healer_name, :patient => patient_name)
       end
       
       combat = TDSCombat.combat(healer_name)
       if (combat)
       ...
       t('tdscombat.treat_success', :healer => healer_name, :patient => patient_name)
     end
     ...
     def self.heal(wound, points)
       ...
       if (healing <= 0)
         new_severity_index = TDSCombat.damage_severities.index(wound.current_severity) - 1
         new_severity = TDSCombat.damage_severities[new_severity_index]
         wound.update(current_severity: new_severity)
         wound.update(healing_points: TDSCombat.healing_points(new_severity))
       else
