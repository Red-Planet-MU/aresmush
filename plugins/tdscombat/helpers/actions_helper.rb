module AresMUSH
  module TDSCombat
    ...
    def self.action_klass_map
      ...
      if (TDSCombat.custom_actions)
        TDSCombat.custom_actions.each do |k, v|
          actions[k] = v
        end
      end
    ...
    def self.find_action_klass(name)
      TDSCombat.action_klass_map[name]
    end
    
    def self.find_action_name(klass)
      if (klass.class == String)
        klass = TDSCombat.const_get(klass)
      end
      
      TDSCombat.action_klass_map.each do |name, value|
        if (value == klass)
          return name
        end
      end
    ...
    def self.damage_table
      Global.read_config("tdscombat", "damage_table")
    end
    
    def self.reset_for_new_turn(combatant)
      TDSCombat.custom_new_turn_reset(combatant)
      
      # Reset aim if they've done anything other than aiming. 
      if (combatant.is_aiming? && combatant.action_klass != "AresMUSH::TDSCombat::AimAction")
      ...
      if (combatant.is_ko && combatant.is_npc?)
        TDSCombat.leave_combat(combatant.combat, combatant)
      else
    ...
    def self.check_for_ko(combatant)
      ...
      if (combatant.is_npc? && (combatant.total_damage_mod <= -7))
        ...
        roll = TDSCombat.make_ko_roll(combatant)
      end
      ...
      if (roll <= 0)
        ...
        TDSCombat.emit_to_combat combatant.combat, t('tdscombat.is_koed', :name => combatant.name, :damaged_by => damaged_by), nil, true
      end
    end
      
    def self.check_for_unko(combatant)
      return if !combatant.is_ko
      roll = TDSCombat.make_ko_roll(combatant, 3)
      
      if (roll > 0)
        combatant.update(is_ko: false)
        TDSCombat.emit_to_combat combatant.combat, t('tdscombat.is_no_longer_koed', :name => combatant.name), nil, true
      end
    end
    
    def self.make_ko_roll(combatant, ko_mod = 0)
      pc_mod = combatant.is_npc? ? 0 : Global.read_config("tdscombat", "pc_knockout_bonus")

      composure = Global.read_config("tdscombat", "composure_skill")
      
      if (combatant.is_in_vehicle?)
        vehicle = combatant.vehicle
        vehicle_mod = TDSCombat.vehicle_stat(vehicle.vehicle_type, "toughness").to_i
      else
        vehicle_mod = 0
      end
    ...
    def self.ai_action(combat, combatant, enactor = nil)
      if (combatant.is_subdued?)
        TDSCombat.set_action(enactor, combat, combatant, TDSCombat::EscapeAction, "")
      elsif (!TDSCombat.check_ammo(combatant, 1))
        TDSCombat.set_action(enactor, combat, combatant, TDSCombat::ReloadAction, "")
      else
        target = TDSCombat.find_ai_target(combat, combatant)
        if (target)
          weapon_type = TDSCombat.weapon_stat(combatant.weapon, "weapon_type")
          case weapon_type
          when "Explosive"
            action_klass = TDSCombat::ExplodeAction
          when "Suppressive"
            action_klass = TDSCombat::SuppressAction
          else
            action_klass = TDSCombat::AttackAction
          end
          TDSCombat.set_action(enactor, combat, combatant, action_klass, target.name)
        else
          TDSCombat.set_action(enactor, combat, combatant, TDSCombat::PassAction, "")
        end
      end   
    end
    ...
    def self.set_action(enactor, combat, combatant, action_klass, args)
      ...
      TDSCombat.emit_to_combat combat, "#{action.print_action}", TDSCombat.npcmaster_text(combatant.name, enactor)      
      return nil
    end
    ...
    def self.determine_damage(combatant, hitloc, weapon, mod = 0, crew_hit = false)
      ...
      lethality = TDSCombat.weapon_stat(weapon, "lethality")
                  
      case TDSCombat.hitloc_severity(combatant, hitloc, crew_hit)
      when "Critical"
        severity = 30
      when "Vital"
        severity = 0
      ...
      if (total < TDSCombat.damage_table["GRAZE"])
        damage = "GRAZE"
      elsif (total < TDSCombat.damage_table["FLESH"])
        damage = "FLESH"
      elsif (total < TDSCombat.damage_table["IMPAIR"])
        damage = "IMPAIR"
      else
        damage = "INCAP"
      end
    ...
    def self.determine_armor(combatant, hitloc, weapon, attacker_net_successes, crew_hit = false)
      ...
      pen = TDSCombat.weapon_stat(weapon, "penetration")
      protect = TDSCombat.armor_stat(armor, "protection")[hitloc]
    ...
    def self.resolve_mount_ko(target)
      toughness = TDSCombat.mount_stat(target.mount_type, 'toughness').to_i
    ...
    def self.determine_attack_margin(combatant, target, mod = 0, called_shot = nil, mount_hit = false)
      ...
      attack_roll = TDSCombat.roll_attack(combatant, target, mod - combatant.recoil)
      defense_roll = TDSCombat.roll_defense(target, weapon)
      
      attacker_net_successes = attack_roll - defense_roll
      stopped_by_cover = target.stance == "Cover" ? TDSCombat.stopped_by_cover?(attacker_net_successes, combatant) : false
      hit = false
      weapon_type = TDSCombat.weapon_stat(combatant.weapon, "weapon_type")
      hit_mount = TDSCombat.hit_mount?(combatant, target, attacker_net_successes, mount_hit)
      
      if (attack_roll <= 0)
        message = t('tdscombat.attack_missed', :name => combatant.name, :target => target.name, :weapon => weapon)
      elsif (called_shot && (attacker_net_successes > 0) && (attacker_net_successes < 2))
        message = t('tdscombat.attack_near_miss', :name => combatant.name, :target => target.name, :weapon => weapon)
      elsif (hit_mount)
        mount_ko = TDSCombat.resolve_mount_ko(target)
        if (mount_ko)
          
          mount_effect = t('tdscombat.mount_ko')
          target.inflict_damage('IMPAIR', 'Fall Damage', true, false)
          target.update(mount_type: nil)
        else
          mount_effect =  t('tdscombat.mount_injured')
        end

        message = t('tdscombat.attack_hits_mount', :name => combatant.name, :target => target.name, :weapon => weapon, :effect => mount_effect)
      elsif (stopped_by_cover)
        message = t('tdscombat.attack_hits_cover', :name => combatant.name, :target => target.name, :weapon => weapon)
        ...
        if (weapon_type == 'Melee' || target.is_in_vehicle?)
          if (attacker_net_successes < -2)
            message = t('tdscombat.attack_dodged_easily', :name => combatant.name, :target => target.name, :weapon => weapon)
          else
            message = t('tdscombat.attack_dodged', :name => combatant.name, :target => target.name, :weapon => weapon)
          end
        else
            message = t('tdscombat.attack_near_miss', :name => combatant.name, :target => target.name, :weapon => weapon)
        end
    ...
    def self.attack_target(combatant, target, mod = 0, called_shot = nil, crew_hit = false, mount_hit = false)
      return [ t('tdscombat.has_no_target', :name => combatant.name) ] if !target
      ...
      margin = TDSCombat.determine_attack_margin(combatant, target, mod, called_shot, mount_hit)

      # Update recoil after determining the attack success but before returning out for a miss
      recoil = TDSCombat.weapon_stat(combatant.weapon, "recoil")
      ...
      TDSCombat.resolve_attack(combatant, combatant.name, target, weapon, attacker_net_successes, called_shot, crew_hit)
    end
    ...
    def self.resolve_attack(attacker, attack_name, target, weapon, attacker_net_successes = 0, called_shot = nil, crew_hit = false)
      hitloc = TDSCombat.determine_hitloc(target, attacker_net_successes, called_shot, crew_hit)
      armor = TDSCombat.determine_armor(target, hitloc, weapon, attacker_net_successes, crew_hit)
        
      
      if (armor >= 100)
        message = t('tdscombat.attack_stopped_by_armor', :name => attack_name, :weapon => weapon, :target => target.name, :hitloc => hitloc) 
        return [message]
      end
                  
      reduced_by_armor = armor > 0 ? t('tdscombat.reduced_by_armor') : ""
      ...
      melee_damage_mod = 0
      weapon_type = TDSCombat.weapon_stat(weapon, "weapon_type")
      
      if (!weapon_type)
        return [ t('tdscombat.attack_with_missing_weapon', :name => weapon) ]
      end
      
      weapon_type = weapon_type.titlecase
      if (weapon_type == "Melee")
        strength_roll = TDSCombat.roll_strength(attacker)
      ...
      damage = TDSCombat.determine_damage(target, hitloc, weapon, total_damage_mod, crew_hit)
          
      is_stun = TDSCombat.weapon_is_stun?(weapon)
      desc = "#{weapon} - #{hitloc}"
      ...
      if (weapon_type == "Explosive")
        weapon_name = t('tdscombat.concussion_from', :weapon => weapon)
      else
        weapon_name = weapon
      end
      
      TDSCombat.award_hit_achievement(attacker, damage, weapon_type)
      
      messages << t('tdscombat.attack_hits', 
                    :name => attack_name, 
                    :weapon => weapon_name,
                    :target => target.name,
                    :hitloc => hitloc,
                    :armor => reduced_by_armor,
                    :damage => TDSCombat.display_severity(damage)) 

      messages.concat TDSCombat.resolve_possible_crew_hit(target, hitloc, damage)
      messages
    end
    ...
    def self.resolve_possible_crew_hit(target, hitloc, damage)
      ...
      crew_hitlocs = TDSCombat.hitloc_chart(target)["crew_areas"]
      ...
      people.each do |p|
        ...
        shrapnel.times.each do |s|
          messages.concat TDSCombat.resolve_attack(nil, t('tdscombat.crew_hit'), p, "Shrapnel", 0, nil, true)
        end
      end
    ...
    def self.resolve_explosion(combatant, target)
      messages = []
      margin = TDSCombat.determine_attack_margin(combatant, target)
      if (margin[:hit])
        attacker_net_successes = margin[:attacker_net_successes]
        messages.concat TDSCombat.resolve_attack(combatant, combatant.name, target, combatant.weapon, attacker_net_successes)
      ...
      if (TDSCombat.weapon_stat(combatant.weapon, "has_shrapnel"))      
        shrapnel = rand(max_shrapnel)
        shrapnel.times.each do |s|
          messages.concat TDSCombat.resolve_attack(nil, combatant.name, target, "Shrapnel")
        end
      end
      messages
    end
