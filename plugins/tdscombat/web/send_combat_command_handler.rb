  ...
  module TDSCombat
    ...
    class SendCombatCommandRequestHandler
      def handle(request)
        ...
        handler_class = TDSCombat.get_cmd_handler(client, cmd, sender)
        
        if (!handler_class || handler_class == CombatHudCmd)
          return { error: t('tdscombat.unrecognized_combat_cmd') }
        end
        ...
        { 
          ...
          data: TDSCombat.build_combat_web_data(combat, enactor)
        }
