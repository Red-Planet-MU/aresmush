module AresMUSH
  module TDSCombat
    class CombatHudTemplate < ErbTemplateRenderer
      ...
      def organizer
        t('tdscombat.combat_hud_organizer', :org => @combat.organizer.name)
      end
      
      def title
        scene = @combat.scene ? @combat.scene.id : t('global.none')
        t('tdscombat.combat_hud_header', :num => @combat.id, :scene => scene)
      end
      ...
      def pilot(v)
        v.pilot ? v.pilot.name : t('tdscombat.no_pilot')
      end
      ...
      def format_vehicle(c)
        if (c.piloting)
          text = t('tdscombat.piloting_hud', :vehicle => c.piloting.name)
        elsif (c.riding_in)
          text = t('tdscombat.passenger_hud', :vehicle => c.riding_in.name)
        else
         text =  ""
        end
        text
      ...
      def format_damage(c)
        return "%xh%xr#{t('tdscombat.ko_status')}%xn" if c.is_ko
        TDSCombat.print_damage(c.total_damage_mod)
      end
