  ...
  module TDSCombat
    describe Combatant do
      ...
      describe :roll_attack do
        before do
          allow(TDSCombat).to receive(:weapon_stat).with("Knife", "skill") { "Knives" }
          allow(TDSCombat).to receive(:weapon_stat).with("Knife", "accuracy") { 0 }
        ...
        it "should roll the weapon attack stat" do
          ...
          TDSCombat.roll_attack(@combatant, @target)
        end
        ...
        it "should give a bonus when mounted vs unmounted target" do
          ...
          allow(TDSCombat).to receive(:mount_stat).with("Horse", "mod_vs_unmounted") { 2 } 
        ...
        it "should account for accuracy modifiers" do
          allow(TDSCombat).to receive(:weapon_stat).with("Knife", "accuracy") { 2 }
          expect(@combatant).to receive(:roll_ability).with("Knives", 2)
          TDSCombat.roll_attack(@combatant, @target)
        end
        ...
        it "should account for passed-in modifiers" do
          expect(@combatant).to receive(:roll_ability).with("Knives", -2)
          TDSCombat.roll_attack(@combatant, @target, -2)
        end
        ...
        it "should account for multiple modifiers" do
          ...
          allow(TDSCombat).to receive(:weapon_stat).with("Knife", "accuracy") { 2 }
          expect(@combatant).to receive(:roll_ability).with("Knives", 2)
          TDSCombat.roll_attack(@combatant, @target, 1)
        end
      end
      ...
      describe :roll_defense do
        before do
          ...
          allow(TDSCombat).to receive(:weapon_defense_skill) { "Reaction" }
          allow(TDSCombat).to receive(:vehicle_dodge_mod) { 0 }
          allow(TDSCombat).to receive(:armor_stat) { 0 }
        end
        
        it "should roll the weapon defense stat" do
          expect(TDSCombat).to receive(:weapon_defense_skill).with(@combatant, "Knife") { "Reaction" }
          expect(@combatant).to receive(:roll_ability).with("Reaction", 0)
          TDSCombat.roll_defense(@combatant, "Knife")
        end
        ...
        it "should account for vehicle dodge modifiers" do
          expect(TDSCombat).to receive(:vehicle_dodge_mod).with(@combatant) { 1 }
          expect(@combatant).to receive(:roll_ability).with("Reaction", 1)
          TDSCombat.roll_defense(@combatant, "Knife")
        end
        ...
        it "should account for armor defense stat" do
          expect(TDSCombat).to receive(:armor_stat).with("armor", "defense") { 1 }
          expect(@combatant).to receive(:roll_ability).with("Reaction", 1)
          TDSCombat.roll_defense(@combatant, "Knife")
        end
      ...
      describe :weapon_defense_skill do
        describe "soldiers" do
          before do
            allow(TDSCombat).to receive(:weapon_stat).with("Sword", "weapon_type") { "Melee" }
            allow(TDSCombat).to receive(:weapon_stat).with("Sword", "skill") { "Swords" }
            allow(TDSCombat).to receive(:weapon_stat).with("Knife", "weapon_type") { "Melee" }
            allow(TDSCombat).to receive(:weapon_stat).with("Knife", "skill") { "Knives" }
            allow(TDSCombat).to receive(:weapon_stat).with("Pistol", "weapon_type") { "Ranged" }
            allow(TDSCombat).to receive(:weapon_stat).with("Pistol", "skill") { "Firearms" }
            allow(TDSCombat).to receive(:combatant_type_stat).with("Soldier", "defense_skill") { "Reaction" }
          ...
          it "should use defender melee skill for melee vs melee" do
            allow(@combatant).to receive(:weapon) { "Knife" }
            expect(TDSCombat.weapon_defense_skill(@combatant, "Sword")).to eq "Knives"
          end
        
          it "should use defender type default for melee vs ranged" do
            allow(@combatant).to receive(:weapon) { "Pistol" }
            expect(TDSCombat.weapon_defense_skill(@combatant, "Sword")).to eq "Reaction"
          end
          
          it "should use defender type default for ranged vs melee" do
            allow(@combatant).to receive(:weapon) { "Knife" }
            expect(TDSCombat.weapon_defense_skill(@combatant, "Pistol")).to eq "Reaction"
          end
          
          
          it "should use the global default if no defender type specified" do
            allow(@combatant).to receive(:combatant_type) { "NoDefense" }
            allow(TDSCombat).to receive(:combatant_type_stat).with("NoDefense", "defense_skill") { nil }
            allow(Global).to receive(:read_config).with("tdscombat", "default_defense_skill") { "Other" }
            allow(@combatant).to receive(:weapon) { "Pistol" }
            expect(TDSCombat.weapon_defense_skill(@combatant, "Sword")).to eq "Other"
          end
          ...
          it "should use piloting skill if in a vehicle" do
            ...
            allow(TDSCombat).to receive(:vehicle_stat).with("Viper", "pilot_skill") { "Piloting" }
            expect(TDSCombat.weapon_defense_skill(@combatant, "Pistol")).to eq "Piloting"
          end
        end            
      ...
      describe :hitloc_chart do
        ...
        it "should use a soldier's hitloc chart" do
          expect(TDSCombat).to receive(:combatant_type_stat).with("soldier", "hitloc") { "human" }
          expect(TDSCombat).to receive(:hitloc_chart_for_type).with("human") { @hitloc }
          expect(TDSCombat.hitloc_chart(@combatant)).to eq @hitloc
        end
        
        it "should use a pilot's vehicle's hitloc chart if not a crew hit" do
          allow(@combatant).to receive(:vehicle) { @vehicle }
          allow(@vehicle).to receive(:hitloc_type) { "fighter" }
          expect(TDSCombat).to receive(:hitloc_chart_for_type).with("fighter") { @hitloc }
          expect(TDSCombat.hitloc_chart(@combatant)).to eq @hitloc
        end
        ...
        it "should use the pilot's hitloc chart if a crew hit" do
          ...
          expect(TDSCombat).to receive(:combatant_type_stat).with("soldier", "hitloc") { "human" }
          expect(TDSCombat).to receive(:hitloc_chart_for_type).with("human") { @hitloc }
          expect(TDSCombat.hitloc_chart(@combatant, true)).to eq @hitloc
        end
      end
      
      describe :hitloc_areas do
        it "should extract areas from the hitloc chart" do
          expect(TDSCombat).to receive(:hitloc_chart).with(@combatant, true) { { "areas" => "x" } }
          expect(TDSCombat.hitloc_areas(@combatant, true)).to eq "x"
        end
      end
    
      describe :hitloc_severity do
        before do 
          allow(TDSCombat).to receive(:hitloc_chart).with(@combatant, false) { 
            { "vital_areas" => [ "Abdomen" ], 
              "critical_areas" => ["head"] }}
        ...
        it "should determine severity for a critical area" do
          expect(TDSCombat.hitloc_severity(@combatant, "Head")).to eq "Critical"
        end

        it "should determine severity for a vital area" do
          expect(TDSCombat.hitloc_severity(@combatant, "ABDOMEN")).to eq "Vital"
        end

        it "should determine severity for a regular area" do
          expect(TDSCombat.hitloc_severity(@combatant, "Arm")).to eq "Normal"
        end
      end
      
      describe :determine_hitloc do 
        before do 
          allow(TDSCombat).to receive(:hitloc_areas) { {
            "Chest" => [ "A", "B", "C" ],
            "Head" => [ "D", "E", "F", "G" ]
          }}
        end
        ...
        it "should work with random lowest value" do
          expect(TDSCombat).to receive(:rand).with(3) { 0 }
          expect(TDSCombat.determine_hitloc(@combatant, 0)).to eq "A"
        end
        
        it "should work with random highest value" do
          expect(TDSCombat).to receive(:rand).with(3) { 2 }
          expect(TDSCombat.determine_hitloc(@combatant, 0)).to eq "C"
        end
        
        it "should add in successes with lowest random value" do
          expect(TDSCombat).to receive(:rand).with(3) { 0 }
          expect(TDSCombat.determine_hitloc(@combatant, 1)).to eq "B"
        end
        
        it "should add in successes with highest random value" do
          expect(TDSCombat).to receive(:rand).with(3) { 2 }
          expect(TDSCombat.determine_hitloc(@combatant, 1)).to eq "C"
        end
        
        it "should work with lowest value and negative modifier" do
          expect(TDSCombat).to receive(:rand).with(3) { 0 }
          expect(TDSCombat.determine_hitloc(@combatant, -2)).to eq "A"
        end
        
        it "should return the exact hit location if called shot and net >= 3" do
          expect(TDSCombat.determine_hitloc(@combatant, 3, "Head")).to eq "Head"
        end
        
        it "should use the called shot hitloc chart if called shot and net < 3" do
          expect(TDSCombat).to receive(:rand).with(4) { 0 }
          expect(TDSCombat.determine_hitloc(@combatant, 2, "Head")).to eq "F"
        end
      end
      ...
      describe :roll_initiative do
        before do
          ...
          allow(TDSCombat).to receive(:weapon_stat) { 0 }
        ...
        it "should roll the init ability" do
          ...
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 3
        end
        ...
        it "should ignore luck spent on something else" do
          ...
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 1
        end
        
        it "should add in a weapon modifier" do 
          allow(@combatant).to receive(:weapon) { "Rapier" }
          expect(TDSCombat).to receive(:weapon_stat).with("Rapier", "init_mod") { 2 }
        ...
        it "should default to 0 for weapon mod" do
          allow(@combatant).to receive(:weapon) { "Rapier" }
          expect(TDSCombat).to receive(:weapon_stat).with("Rapier", "init_mod") { nil }
          expect(@combatant).to receive(:roll_ability).with("init", 0) { 1 }
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 1
        end

        it "should apply mod for subdue action" do
          allow(@combatant).to receive(:action_klass) { "AresMUSH::TDSCombat::SubdueAction" }
          expect(@combatant).to receive(:roll_ability).with("init", 3) { 4 }                  
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 4
        end
                
        it "should apply mod for suppress action" do
          allow(@combatant).to receive(:action_klass) { "AresMUSH::TDSCombat::SuppressAction" }
          expect(@combatant).to receive(:roll_ability).with("init", 3) { 4 }                  
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 4
        end
        
        it "should apply mod for distract action" do
          allow(@combatant).to receive(:action_klass) { "AresMUSH::TDSCombat::DistractAction" }
          expect(@combatant).to receive(:roll_ability).with("init", 3) { 4 }
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 4
        end
        
        it "should not apply mod for regular actions" do
          allow(@combatant).to receive(:action_klass) { "AresMUSH::TDSCombat::AttackAction" }
          expect(@combatant).to receive(:roll_ability).with("init", 0) { 2 }
          expect(TDSCombat.roll_initiative(@combatant, "init")).to eq 2
        end
      ...
      describe :check_ammo do
        ...
        it "should return true if the weapon doesn't use ammo" do
          ...
          expect(TDSCombat.check_ammo(@combatant, 22)).to be true          
        end
         
        it "should return true if enough bullets" do
          allow(@combatant).to receive(:ammo) { 10 }
          expect(TDSCombat.check_ammo(@combatant, 2)).to be true
        end
        
        it "should return false if not enough bullets" do
          allow(@combatant).to receive(:ammo) { 10 }
          expect(TDSCombat.check_ammo(@combatant, 22)).to be false
        end
      end
      ...
      describe :update_ammo do
        ...
        it "should not do anything if the weapon doesn't use ammo" do
          ...
          expect(TDSCombat.update_ammo(@combatant, 1)).to be_nil
        end
        ...
        it "should adjust ammo by the number of bullets" do
          ...
          expect(TDSCombat.update_ammo(@combatant, 3)).to be_nil
        end
      
        it "should warn if out of ammo" do
          allow(@combatant).to receive(:ammo) { 4 }
          expect(@combatant).to receive(:update).with(ammo: 0)
          expect(TDSCombat.update_ammo(@combatant, 4)).to eq "tdscombat.weapon_clicks_empty"
        end
      end
    end
  end
