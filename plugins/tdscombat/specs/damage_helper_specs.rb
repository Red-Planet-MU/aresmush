module AresMUSH
  module TDSCombat
    describe TDSCombat do
      ...
      before do
        allow(Global).to receive(:read_config).with("tdscombat", "recovery_skill") { "Body" }
        allow(Global).to receive(:read_config).with("tdscombat", "treat_skill") { "First Aid" }
        stub_translate_for_testing        
      end
        
      describe :print_damage do
        it "should print the right damage" do
          expect(TDSCombat.print_damage(-0)).to eq "%xr%xn----"
          expect(TDSCombat.print_damage(-0.25)).to eq "%xrX%xn---"
          expect(TDSCombat.print_damage(-1.0)).to eq "%xrX%xn---"
          expect(TDSCombat.print_damage(-1.25)).to eq "%xrXX%xn--"
          expect(TDSCombat.print_damage(-2.25)).to eq "%xrXXX%xn-"
          expect(TDSCombat.print_damage(-3.25)).to eq "%xrXXXX%xn"
          expect(TDSCombat.print_damage(-5)).to eq "%xrXXXX%xn"
        end
      end
      ...
      describe :total_damage_mod do
        it "should count all wound levels" do
          ...
          expect(TDSCombat.total_damage_mod(char)).to eq -3
        end

        it "should count empty damage" do
          char = double
          allow(char).to receive(:damage) { [] }
          expect(TDSCombat.total_damage_mod(char)).to eq 0
        end        
      end
      ...
      describe :heal_wounds do
        ...
        it "should apply a base of 1 healing point to each wound" do
          expect(TDSCombat).to receive(:heal).with(@damage1, 1)
          expect(TDSCombat).to receive(:heal).with(@damage2, 1)
          TDSCombat.heal_wounds(@char)
        end
        ...
        it "should add a recovery roll to the healing points" do
          ...
          expect(TDSCombat).to receive(:heal).with(@damage1, 2)
          expect(TDSCombat).to receive(:heal).with(@damage2, 2)
          TDSCombat.heal_wounds(@char)
        end        
      ...
      describe :heal do
        before do
          allow(Global).to receive(:read_config).with("tdscombat", "healing_points", "FLESH") { 5 }
          allow(Global).to receive(:read_config).with("tdscombat", "healing_points", "HEAL") { 0 }
          @damage = double
      ...
      describe :worst_treatable_wound do
        ...
        it "should return the most serious wound if all treatable" do          
          ...
          expect(TDSCombat.worst_treatable_wound(@char)).to eq @damage2
        end
        
        it "should ignore treated wounds" do
          allow(@damage2).to receive(:is_treatable?) { false }
          allow(@char).to receive(:damage) { [@damage1, @damage2, @damage3] }
          expect(TDSCombat.worst_treatable_wound(@char)).to eq @damage3
        end
        ...
        it "should return nil if no treatable wounds" do
          ...
          allow(@char).to receive(:damage) { [@damage1, @damage2, @damage3] }
          expect(TDSCombat.worst_treatable_wound(@char)).to be_nil
        end
      ...
      describe :treat do
        before do 
          ...
          allow(TDSCombat).to receive(:combat)
          @damage = double
          allow(TDSCombat).to receive(:worst_treatable_wound) { @damage }
        end
        
        it "should not do anything if no treatable wounds" do
          expect(TDSCombat).to receive(:worst_treatable_wound).with(@patient) { nil }
          expect(TDSCombat).to_not receive(:heal)
          expect(TDSCombat.treat(@patient, @doctor)).to eq "tdscombat.no_treatable_wounds"
        end
        
        it "should add a healing point if treat roll successful" do
          allow(Global).to receive(:read_config).with("tdscombat", "treat_skill") { "Medicine" }
          expect(@doctor).to receive(:roll_ability).with("Medicine") { { successes: 2 }}
          
          expect(TDSCombat).to receive(:heal).with(@damage, 1)
          expect(TDSCombat.treat(@patient, @doctor)).to eq "tdscombat.treat_success"
        end
        ...
        it "should not heal if treat roll unsuccessful" do
          ...
          expect(TDSCombat).to_not receive(:heal)
          expect(TDSCombat.treat(@patient, @doctor)).to eq "tdscombat.treat_failed"
        end
      end
    end
  end
