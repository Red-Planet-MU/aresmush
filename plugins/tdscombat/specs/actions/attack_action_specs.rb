    ...
    describe AttackAction do
      ...
      describe :prepare do
        ...
        it "should parse target plus called" do
          ...
          allow(TDSCombat).to receive(:has_hitloc?).with(@target, "Head") { true }
          expect(@action.prepare).to be_nil
          expect(@action.is_burst).to be false
        ...
        it "should raise error for invalid called shot location" do
          allow(TDSCombat).to receive(:has_hitloc?).with(@target, "Head") { false }
          @action = AttackAction.new(@combatant, "target/called:head")
          expect(@action.prepare).to eq "tdscombat.invalid_called_shot_loc"
        end
        
        it "should raise error for invalid special" do
          @action = AttackAction.new(@combatant, "target/foo:3")
          expect(@action.prepare).to eq "tdscombat.invalid_attack_special"
        end
        
        it "should fail if multiple targets" do
          @action = AttackAction.new(@combatant, "target1 target2")
          expect(@action.prepare).to eq "tdscombat.only_one_target"
        end
        
        it "should fail if out of ammo" do
          expect(TDSCombat).to receive(:check_ammo).with(@combatant, 1) { false }
          @action = AttackAction.new(@combatant, "target")
          expect(@action.prepare).to eq "tdscombat.out_of_ammo"
        end

        it "should fail if not enough ammo for a burst" do
          expect(TDSCombat).to receive(:check_ammo).with(@combatant, 2) { false }
          @action = AttackAction.new(@combatant, "target/burst")
          expect(@action.prepare).to eq "tdscombat.not_enough_ammo_for_burst"
        end
        ...
        it "should fail if trying to burst with called shot" do
          ...
          expect(@action.prepare).to eq "tdscombat.no_fullauto_called_shots"
        end
      ...
      describe :resolve do
        ...
        it "should attack in single fire" do
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target, 0, nil, false, false) { ["result1"] }
        ...
        it "should attack in burst fire" do
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target, 0, nil, false, false) { ["result1"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target, 0, nil, false, false) { ["result2"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target, 0, nil, false, false) { ["result3"] }
          @action.is_burst = true
          resolutions = @action.resolve
          expect(resolutions[0]).to eq "tdscombat.fires_burst"
        ...
        it "should update ammo for a single shot" do
          expect(TDSCombat).to receive(:update_ammo).with(@combatant, 1)
          @action.resolve
        end
        ...
        it "should update ammo for a burst" do
          ...
          expect(TDSCombat).to receive(:update_ammo).with(@combatant, 3)
          @action.resolve
        end
        ...
        it "should update ammo for a burst with limited ammo" do
          ...
          @action.is_burst = true
          allow(@combatant).to receive(:attack_target) { "result" }
          expect(TDSCombat).to receive(:update_ammo).with(@combatant, 2)
          @action.resolve
        end
        
        it "should add an out of ammo message" do
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target, 0, nil, false, false) { ["result1"] }
          expect(TDSCombat).to receive(:update_ammo).with(@combatant, 1) { "out of ammo" }
          resolutions = @action.resolve
          expect(resolutions[0]).to eq "result1"
