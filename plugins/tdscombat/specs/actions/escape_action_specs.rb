    ...
    describe EscapeAction do
      ...
      describe :prepare do
        ...
        it "should fail if not subdued" do
          ...
          expect(@action.prepare).to eq "tdscombat.not_subdued"
        end
      ...
      describe :resolve do
        ...
        it "should escape automatically if subduer is no longer subduing" do
          ...
          expect(resolutions).to eq [ "tdscombat.escape_action_success" ]
        end
        ...
        it "should escape with a failed subdue roll" do
          ...
          allow(TDSCombat).to receive(:determine_attack_margin).with(@subduer, @combatant) { {hit: false} }
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.escape_action_success" ]
        end
        ...
        it "should stay subdued with a successful subdue roll" do
          ...
          allow(TDSCombat).to receive(:determine_attack_margin).with(@subduer, @combatant) { {hit: true} }
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.escape_action_failed" ]
        end
