  ...
  module TDSCombat
    describe SubdueAction do
      before do
        ...
        allow(TDSCombat).to receive(:weapon_stat).with("Unarmed", "weapon_type") { "Melee" }
      ...
      describe :prepare do
        ...
        it "should fail if not a melee weapon" do
          expect(TDSCombat).to receive(:weapon_stat).with("Unarmed", "weapon_type") { "Explosive" }
          @action = SubdueAction.new(@combatant, "target1")
          expect(@action.prepare).to eq "tdscombat.subdue_uses_melee"
        end
      ...
      describe :resolve do
        ...
        it "should subdue successfully" do
          ...
          expect(TDSCombat).to receive(:determine_attack_margin).with(@combatant, @target1) { results }
          ...
          expect(resolutions).to eq [ "tdscombat.subdue_action_success" ]
        end

        it "should subdue unsuccessfully" do
          @action = SubdueAction.new(@combatant, "target1")
          @action.prepare
          expect(TDSCombat).to receive(:determine_attack_margin).with(@combatant, @target1) { { hit: false } }
          expect(@target1).to_not receive(:update)
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.subdue_action_failed" ]
        end
        ...
        it "should continue subduing without rolling" do
          ...
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.continues_subduing" ]
        end
      end
    end
  end
