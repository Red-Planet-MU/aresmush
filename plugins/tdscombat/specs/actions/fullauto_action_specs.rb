  ...
  module TDSCombat
    describe FullautoAction do
      before do
        ...
        allow(TDSCombat).to receive(:update_ammo) { nil }
      ...
      describe :prepare do
        ...
        it "should fail if too many targets" do
          ...
          expect(@action.prepare).to eq "tdscombat.too_many_targets"
        end
        
        it "should fail if not enough ammo for a burst" do
          expect(TDSCombat).to receive(:check_ammo).with(@combatant, 8) { false }
          @action = FullautoAction.new(@combatant, "target")
          expect(@action.prepare).to eq "tdscombat.not_enough_ammo_for_fullauto"
        end

        it "should fail if trying to burst with non-auto weapon" do
          expect(TDSCombat).to receive(:weapon_stat).with("Rifle", "is_automatic") { false }
          @action = FullautoAction.new(@combatant, "target")
          expect(@action.prepare).to eq "tdscombat.burst_fire_not_allowed"
        end
        ...
        it "should warn if using an explosive weapon" do
          ...
          expect(@action.prepare).to eq "tdscombat.use_explode_command"
        end
        ...
        it "should warn if using a suppressive weapon" do
          ...
          @action = FullautoAction.new(@combatant, "target1 target2")
          expect(@action.prepare).to eq "tdscombat.use_suppress_command"
        end
      ...
      describe :resolve do
        before do
          ...
          allow(TDSCombat).to receive(:attack_target) { ["resultx"] }
        end
          
        it "should attack a single target with all the bullets" do
          @action = FullautoAction.new(@combatant, "target1")
          @action.prepare
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result1"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result2"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result3"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result4"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result5"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result6"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result7"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result8"] }
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.fires_fullauto", "result1", "result2", "result3", "result4",
        ...
        it "should attack two targets with split bullets" do
          ...
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result2"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result3"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target2) { ["result4"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target2) { ["result5"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target2) { ["result6"] }
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.fires_fullauto", "result1", "result2", "result3", "result4",
            "result5", "result6" ]
        end
        ...
        it "should attack three targets with split bullets" do
          ...
          @action.prepare
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target1) { ["result1"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target2) { ["result2"] }
          expect(TDSCombat).to receive(:attack_target).with(@combatant, @target3) { ["result3"] }
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.fires_fullauto", "result1", "result2", "result3" ]
        end
        ...
        it "should update ammo" do
          ...
          expect(TDSCombat).to receive(:update_ammo).with(@combatant, 8)
          @action.resolve
        end
        ...
        it "should add an out of ammo message" do
          @action = FullautoAction.new(@combatant, "target1 target2 target3")
          @action.prepare
          expect(TDSCombat).to receive(:update_ammo).with(@combatant, 8) { "out of ammo" }
          resolutions = @action.resolve
