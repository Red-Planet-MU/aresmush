    ...
    describe SuppressAction do
      ...
      describe :prepare do
        ...
        it "should fail if too many targets for fullauto" do
          expect(TDSCombat).to receive(:weapon_stat).with("Rifle", "weapon_type") { "Ranged" }
          expect(TDSCombat).to receive(:weapon_stat).with("Rifle", "is_automatic") { true }
      ...
      describe :resolve do
        ...
        it "should suppress a single target successfully" do
          ...
          @action.prepare
          expect(TDSCombat).to receive(:roll_attack).with(@combatant, @target1) { 2 }
          ...
          expect(resolutions).to eq [ "tdscombat.suppress_successful_msg" ]
        end

        it "should suppress a single target unsuccessfully" do
          @action = SuppressAction.new(@combatant, "target1")
          @action.prepare
          expect(TDSCombat).to receive(:roll_attack).with(@combatant, @target1) { 0 }
          expect(@target1).to receive(:roll_ability).with("Composure") { 1 }
          expect(@target1).to_not receive(:add_stress)
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.suppress_failed_msg" ]
        end
        ...
        it "should suppress multiple targets with separate rolls" do
          ...
          @action.prepare
          expect(TDSCombat).to receive(:roll_attack).with(@combatant, @target1) { 2 }
          expect(TDSCombat).to receive(:roll_attack).with(@combatant, @target2) { 1 }
          expect(@target1).to receive(:roll_ability).with("Composure") { 0 }
          expect(@target2).to receive(:roll_ability).with("Composure") { 3 }
          expect(@target1).to receive(:add_stress).with(4)
          expect(@target2).to_not receive(:add_stress)
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.suppress_successful_msg", "tdscombat.suppress_failed_msg" ]
        end
