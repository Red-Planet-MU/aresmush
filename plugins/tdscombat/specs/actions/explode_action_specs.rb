  ...
  module TDSCombat
    describe ExplodeAction do
      before do
        ...
        allow(TDSCombat).to receive(:weapon_stat).with("Missile", "weapon_type") { "Explosive" }
      ...
      describe :prepare do
        ...
        it "should fail if trying to use a non-explosive weapon" do
          expect(TDSCombat).to receive(:weapon_stat).with("Missile", "weapon_type") { "Ranged" }
          @action = ExplodeAction.new(@combatant, "target")
          expect(@action.prepare).to eq "tdscombat.not_explosive_weapon"
        end
      ...
      describe :resolve do
        ...
        it "should resolve explosive damage on each target" do
          ...
          @action.prepare
          expect(TDSCombat).to receive(:resolve_explosion).with(@combatant, @target1) { ["result1"] }
          expect(TDSCombat).to receive(:resolve_explosion).with(@combatant, @target2) { ["result2"] }
          expect(TDSCombat).to receive(:resolve_explosion).with(@combatant, @target3) { ["result3"] }
          resolutions = @action.resolve
          expect(resolutions).to eq [ "tdscombat.explode_resolution_message", "result1", "result2", "result3" ]
        end
