  ...
  module TDSCombat
    describe TDSCombat do
      ...
      describe :find_combat_by_number do
        ...
        it "should fail if not a number" do
          expect(@client).to receive(:emit_failure).with("tdscombat.invalid_combat_number")
          expect(TDSCombat.find_combat_by_number(@client, "A")).to be_nil
        end
        
        it "should fail if not a valid number" do
          expect(@client).to receive(:emit_failure).with("tdscombat.invalid_combat_number")
          expect(TDSCombat.find_combat_by_number(@client, 3)).to be_nil
        end
        
        it "should succeed if valid number string specified" do
          expect(TDSCombat.find_combat_by_number(@client, "1")).to eq @combat1
          expect(TDSCombat.find_combat_by_number(@client, "2")).to eq @combat2
        end
    ...
    describe :get_initiative_order do
      it "should return combatants in order of initiative roll" do
        ...
        allow(Global).to receive(:read_config).with("tdscombat", "initiative_skill") { "init" }
        
        expect(TDSCombat).to receive(:roll_initiative).with(@combatant1, "init") { 3 }
        expect(TDSCombat).to receive(:roll_initiative).with(@combatant2, "init") { 5 }
        expect(TDSCombat).to receive(:roll_initiative).with(@combatant3, "init") { 1 }
        
        expect(TDSCombat.get_initiative_order(@combat)).to eq [ 2, 1, 3 ]
      end
    end
