  ...
  module TDSCombat
    class CombatWeaponCmd
      ...
      def check_special_allowed
        ...
        allowed_specials = TDSCombat.weapon_stat(self.weapon, "allowed_specials") || []
        self.specials.each do |s|
          return t('tdscombat.invalid_weapon_special', :special => s) if !allowed_specials.include?(s)
        end
        return nil
      end
      
      def check_valid_weapon
        return t('tdscombat.invalid_weapon') if !TDSCombat.weapon(self.weapon)
        return nil
      end
      ...
      def handle
        self.names.each do |name|
          TDSCombat.with_a_combatant(name, client, enactor) do |combat, combatant|        
            TDSCombat.set_weapon(enactor, combatant, self.weapon, self.specials)
          end
        end
      end
    end
