module AresMUSH
  module TDSCombat
    class ModifyDamageCmd
      ...
      def check_severity
        return t('tdscombat.invalid_severity', :severities => TDSCombat.damage_severities.join(" ")) if !TDSCombat.damage_severities.include?(self.current_severity)
        return t('tdscombat.invalid_severity', :severities => TDSCombat.damage_severities.join(" ")) if !TDSCombat.damage_severities.include?(self.initial_severity)
        return nil
      end
      ...
      def check_date
        ...
        if (!day)
          return t('tdscombat.invalid_damage_date', 
      ...
      def handle
        ...
        if (target)
          ...
          wound.update(healing_points: TDSCombat.healing_points(self.current_severity))
          wound.update(ictime_str: self.ictime_str)
          client.emit_success t('tdscombat.damage_modified') 
        else 
