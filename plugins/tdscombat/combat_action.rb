module AresMUSH
  class CombatAction
    ...
    def parse_targets(name_string)
      return t('tdscombat.no_targets_specified') if (name_string.blank?)
      target_names = name_string.split(" ").map { |n| InputFormatter.titlecase_arg(n) }.uniq
      targets = []
      target_names.each do |name|
        target = self.combat.find_combatant(name)
        return t('tdscombat.not_in_combat', :name => name) if !target
        return t('tdscombat.cant_target_noncombatant', :name => name) if target.is_noncombatant?
        targets << target
      end
