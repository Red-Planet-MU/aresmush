  ...
  class Combatant < Ohm::Model
    ...
    def action
      ...
      if (error)
        ...
        #TDSCombat.emit_to_combat self.combat, t('tdscombat.invalid_action', :name => self.name, :error => error)
        return nil
      end
    ...
    def reset_action
      TDSCombat.emit_to_combat self.combat, t('tdscombat.resetting_action', :name => self.name)
    ...
    def is_subduing?(target)
      self.action && self.action.class == TDSCombat::SubdueAction && self.action.targets && self.action.target == target
    end
    
    def is_passing?
      !self.action || self.action.class == TDSCombat::PassAction
    end
    ...
    def total_damage_mod
      if (is_in_vehicle?)
        TDSCombat.total_damage_mod(self.associated_model) + TDSCombat.total_damage_mod(self.vehicle)
      else
        TDSCombat.total_damage_mod(self.associated_model)
      end
    end
    ...
    def inflict_damage(severity, desc, is_stun = false, is_crew_hit = false)
      ...
      TDSCombat.inflict_damage(model, severity, desc, is_stun, !self.combat.is_real)
    end
      
    def attack_stance_mod
      stance_config = TDSCombat.stances[self.stance]
    ...
    def get_action_instance
      return nil if !self.action_klass
      klass = TDSCombat.const_get(self.action_klass)
